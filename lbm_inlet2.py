...
@ti.data_oriented
class LBM:
    def __init__(self, width=1024, height=512, tau=0.8, rho0=1.0, inlet_val=0.1, block_size=128, theta=0):
    ...
    def display(self):
        ...
        drag = []
        lift = []
        while not gui.get_event(ti.GUI.ESCAPE, ti.GUI.EXIT) and step < 6000:
            self.max_vel()
...
#     np.savetxt(f"2drags{final}.csv", drags, delimiter =", ", fmt ='% s')
#     np.savetxt(f"2lifts{final}.csv", lifts, delimiter =", ", fmt ='% s')

drag = []
lift = []

for i in range(4):
    data = np.loadtxt(f"2drags{5 * (i + 1)}.csv", delimiter=",", dtype=float)
    drag.extend(data.tolist())
    data = np.loadtxt(f"2lifts{5 * (i + 1)}.csv", delimiter=",", dtype=float)
